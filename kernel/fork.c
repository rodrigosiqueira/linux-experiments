				    ...
				    unsigned long tls)
{
	...
	if (IS_ERR(p->mempolicy)) {
		retval = PTR_ERR(p->mempolicy);
		p->mempolicy = NULL;
		goto bad_fork_cleanup_threadgroup_lock_2;
	}
#endif
#ifdef CONFIG_CPUSETS
	p->cpuset_mem_spread_rotor = NUMA_NO_NODE;
	...
	retval = audit_alloc(p);
	if (retval)
		goto bad_fork_cleanup_perf_2;
	/* copy all the process information */
	shm_init_task(p);
	retval = security_task_alloc(p, clone_flags);
	if (retval)
		goto bad_fork_cleanup_audit_2;
	retval = copy_semundo(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_security_2;
	retval = copy_files(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_semundo_2;
	retval = copy_fs(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_files_2;
	retval = copy_sighand(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_fs_2;
	retval = copy_signal(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_sighand_2;
	retval = copy_mm(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_signal_2;
	retval = copy_namespaces(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_mm_2;
	retval = copy_io(clone_flags, p);
	if (retval)
		goto bad_fork_cleanup_namespaces_2;
	retval = copy_thread_tls(clone_flags, stack_start, stack_size, p, tls);
	if (retval)
		goto bad_fork_cleanup_io_2;
/*
	if (pid != &init_struct_pid) {
		pid = alloc_pid(p->nsproxy->pid_ns_for_children);
		if (IS_ERR(pid)) {
			retval = PTR_ERR(pid);
			goto bad_fork_cleanup_thread_2;
		}
	}
*/
#ifdef CONFIG_BLOCK
...
bad_fork_free_pid_2:
	...
	//if (pid != &init_struct_pid)
	//	free_pid(pid);
bad_fork_cleanup_thread_2:
	exit_thread(p);
bad_fork_cleanup_io_2:
	if (p->io_context)
		exit_io_context(p);
bad_fork_cleanup_namespaces_2:
	exit_task_namespaces(p);
bad_fork_cleanup_mm_2:
	if (p->mm)
		mmput(p->mm);
bad_fork_cleanup_signal_2:
	if (!(clone_flags & CLONE_THREAD))
		free_signal_struct(p->signal);
bad_fork_cleanup_sighand_2:
	__cleanup_sighand(p->sighand);
bad_fork_cleanup_fs_2:
	exit_fs(p); /* blocking */
bad_fork_cleanup_files_2:
	exit_files(p); /* blocking */
bad_fork_cleanup_semundo_2:
	exit_sem(p);
bad_fork_cleanup_security_2:
	security_task_free(p);
bad_fork_cleanup_audit_2:
	audit_free(p);
bad_fork_cleanup_perf_2:
	perf_event_free_task(p);
bad_fork_cleanup_policy_2:
#ifdef CONFIG_NUMA
	mpol_put(p->mempolicy);
bad_fork_cleanup_threadgroup_lock_2:
#endif
	delayacct_tsk_free(p);
bad_fork_cleanup_count_2:
	atomic_dec(&p->cred->user->processes);
	exit_creds(p);
bad_fork_free_2:
	p->state = TASK_DEAD;
...
pr_info("--- Chama o swith_to");
	switch_to(prev, next, prev);

pr_info("context_switch 2: depois de switch_to --> UNLOCK SOON");
	barrier();
